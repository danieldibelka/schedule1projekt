...
<head>
    ...
    <title>Uživatelé | Váš Databázový Projekt</title>
...
<body>
    ...
    <main>
        <section>
            ...
            $sql = "SELECT id, name, address, weed_quality_requieremt, addiction, pref_effect FROM users";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                echo "<table>";
                echo "<thead>";
                echo "<tr><th>ID</th><th>Jméno</th><th>Adresa</th><th>Požadavek na kvalitu Weedu</th><th>Závislost</th><th>Preferovaný Efekt</th></tr>";
                echo "</thead>";
                echo "<tbody>";
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>" . $row["weed_quality_requieremt"] . "</td>";
                    echo "<td>" . $row["addiction"] . "</td>";
                    echo "<td>" . $row["pref_effect"] . "</td>";
                    echo "</tr>";
                }
                echo "</tbody>";
                echo "</table>";
            } else {
                echo "<p>Žádní uživatelé nebyli nalezeni v databázi.</p>";
            }
